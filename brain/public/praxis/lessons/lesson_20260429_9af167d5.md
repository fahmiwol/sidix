# Pelajaran Praxis — jejak agen SIDIX

**Diperbarui:** 2026-04-29T23:45:38.190015+00:00
**session_id:** `9af167d5`
**persona:** ABOO

## Pertanyaan / tugas pengguna

Apa itu machine learning?

## Rangkaian eksekusi (Thought → Action → Observation)

### Langkah 0

- **Thought:** Pertanyaan faktual umum. Cari konteks di korpus dulu sebelum LLM: 'Apa itu machine learning?'.
- **Action:** `search_corpus` — args: `{"query": "Apa itu machine learning?", "k": 3, "persona": "ABOO", "_citations": [{"n": "1", "source_path": "sources/web_clips/apa-itu-rag-retrieval-augmented-generation-ibm-https-www-ibm-com-id-id-think-topics-retrieval-augmented-generation.md", "source_title": "Ringkasan (draft, offline)", "chunk_id": "sources/web_clips/apa-itu-rag-retrieval-augmented-generation-ibm-https-www-ibm-com-id-id-think-topics-retrieval-augmented-generation.md:0-1200", "sanad_tier": "unknown"}, {"n": "2", "source_path": "coding/roadmap_ml_topics.md", "source_title": "Machine Learning Roadmap — Topic Index + Quick Ref`
- **Observation (cuplikan):** Q: Apa itu machine learning?

Konteks Memori (prinsip & preferensi aktif):
- 📌 Prinsip [Kejujuran & realisme dulu]: Saya lebih suka jawaban yang jujur, realistis, dan bertahap. Kalau sesuatu belum bisa dicapai cepat, jelaskan alternatif yang paling dekat dan langkah kecil yang bisa dilakukan hari ini.
- ⚙️ Preferensi [Jawaban ringkas, actionable]: Saya suka jawaban singkat tapi bisa langsung dieksekusi: checklist, template, dan contoh nyata. Hindari teori panjang tanpa langkah praktis.
- 📌 Prinsip [IHOS: jujur & rendah hati]: Kalau belum ada sumber/pengetahuan, bilang tidak tahu. Pisahkan fakt

### Langkah 1

- **Thought:** Hasil corpus tipis/gagal. Fallback: kutipan Wikipedia (API resmi, allowlist).
- **Action:** `search_web_wikipedia` — args: `{"query": "Apa itu machine learning?", "lang": "id", "_citations": [{"type": "wikipedia", "n": "1", "title": "Pembelajaran mesin", "url": "https://id.wikipedia.org/wiki/Pembelajaran_mesin", "snippet": "Pembelajaran mesin atau pemelajaran mesin (bahasa Inggris: machine learning atau disingkat dengan ML) adalah sub-bidang kecerdasan buatan yang berkaitan", "lang": "id"}, {"type": "wikipedia", "n": "2", "title": "Metode ensemble", "url": "https://id.wikipedia.org/wiki/Metode_ensemble", "snippet": "ensemble atau metode ansamble adalah algoritma dalam pembelajaran mesin (machine learning) di mana a`
- **Observation (cuplikan):** [web:wikipedia:id] Kutipan ringkas hasil pencarian (allowlist: id.wikipedia.org):

**1. Pembelajaran mesin**
- URL: https://id.wikipedia.org/wiki/Pembelajaran_mesin
- Kutipan: Pembelajaran mesin atau pemelajaran mesin (bahasa Inggris: machine learning atau disingkat dengan ML) adalah sub-bidang kecerdasan buatan yang berkaitan

**2. Metode ensemble**
- URL: https://id.wikipedia.org/wiki/Metode_ensemble
- Kutipan: ensemble atau metode ansamble adalah algoritma dalam pembelajaran mesin (machine learning) di mana algoritma ini sebagai pencarian solusi prediksi terbaik dibandingkan

**3. Meta-lear

### Langkah 2

- **Thought:** Sudah ada hasil Wikipedia. Rangkai final answer dengan label sumber web.
- **Final (cuplikan):** Machine learning adalah cabang dari kecerdasan buatan (AI) yang fokus pada pembuatan algoritma dan model statistik yang mampu belajar dari dan membuat prediksi berdasarkan data. Intinya, ML mencoba membuat sistem yang dapat meningkatkan performa mereka dengan memanfaatkan lebih banyak data.

Langkah praktis untuk memulai:
1. **Pahami Konsep Dasar**: Pelajari tentang algoritma pemb

## Jawaban akhir (ringkas)

Machine learning adalah cabang dari kecerdasan buatan (AI) yang fokus pada pembuatan algoritma dan model statistik yang mampu belajar dari dan membuat prediksi berdasarkan data. Intinya, ML mencoba membuat sistem yang dapat meningkatkan performa mereka dengan memanfaatkan lebih banyak data.

Langkah praktis untuk memulai:
1. **Pahami Konsep Dasar**: Pelajari tentang algoritma pemb

## Cuplikan orkestrasi

```text
SIDIX OrchestrationPlan (deterministik)
request_persona=ABOO router=AYMAN
router_reason: score=1; signals=creative/design; conf=0.63

Archetype scores:
  - deduce: 0.000
  - connect: 0.000
  - invent: 0.000
  - synthesize: 0.000
  - orient: 0.350

Satellite weights (inspirasi fiksi / dev labels):
  - edison: 0.0000
  - pythagoras: 0.0000
  - shaka: 0.0000
  - lilith: 0.0000
  - atlas: 0.0000
  - york: 1.0000

Suggested phase order:
  1. [orient] persona~INAN -> Ringkas keputusan untuk pengguna; langkah berikutnya eksplisit.

JSON:
{"request_persona": "ABOO", "router_persona": "AYMAN", "router_reason": "score=1; signals=creative/design; conf=0.63", "archetype_scores": {"deduce": 0.0, "connect": 0.0, "invent": 0.0, "synthesize": 0.0, "orient": 0.35}, "satellite_weights": {"edison": 0.0, "pythagoras": 0.0, "shaka": 0.0, "lilith": 0.0, "atlas": 0.0, "york": 1.0}, "phases": [{"archetype": "orient", "persona": "INAN", "hint": "Ringkas keputusan untuk pengguna; langkah berikutnya eksplisit."}]}
```

## Kerangka kasus (runtime — niat & cabang data)

**Kerangka situasi (niat & inisiasi)** — dari pola terkurasi Praxis, bukan tebak-tebakan bebas:

- **factual_corpus** _(skor 0.42)_ — **Niat:** Menjawab dari landasan yang bisa disanadkan: korpus BM25 dulu; bedakan fakta, opini, dan spekulasi.
  1. Sempitkan pertanyaan ke entitas/konsep utama.
  2. Gunakan search_corpus; baca chunk relevan sebelum menyimpulkan.
  3. Jika jawaban tidak ada di korpus, katakan eksplisit dan sarankan indeks/sumber.
  → **Bila ada cuplikan korpus:** Kutip ringkas + sebut jalur sumber bila tersedia; hindari klaim di luar cuplikan.

_Ini kerangka perilaku yang dapat ditambah di `brain/public/praxis/patterns/case_frames.json`; lesson Markdown mencatat bukti eksekusi nyata._

## Untuk SIDIX — cara berpikir seperti agen eksekutor

1. **Rekam dulu:** salin pertanyaan, persona, dan setiap *thought* sebelum bertindak.
2. **Pilah:** bedakan faktual (butuh korpus) vs meta (orkestrasi) vs implementasi (sandbox).
3. **Pilih alat:** satu tool per langkah; evaluasi observasi sebelum lanjut atau final.
4. **Batasi risiko:** jangan sebar secret; potong observasi panjang; hormati `corpus_only` / web fallback.
5. **Tutup dengan jawaban:** rangkum sumber + langkah berikutnya; akui ketidakpastian bila perlu.

_Tag: #praxis #sidix-agent #meta-learning_
