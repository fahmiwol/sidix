...
- Teks user-facing, landing, dan artefak publik SIDIX: hindari *footprint* nama vendor/asisten/IDE (mis. Cursor, Kimi, Antigravity, Claude); gunakan narasi netral atau metafora.

## Learned Workspace Facts

- Primary workspace: `D:\MIGHAN Model` (PowerShell); clone/sesi aktif sering `C:\SIDIX-AI` (selaras GitHub `fahmiwol/sidix`). Orientasi: `docs/00_START_HERE.md`; snapshot `docs/STATUS_TODAY.md`.
- **Fondasi SIDIX / IHOS (referensi singkat):** `docs/SIDIX_FUNDAMENTALS.md` — produk, definisi IHOS vs mnemonik feeding, lapisan AI; pelengkap praktis: `docs/SIDIX_CODING_CURRICULUM_V1.md`. **Istilah produk (dokumentasi/sprint, publik):** Maqashid, Naskh, Raudah, Sanad, Muhasabah, Jariyah, Tafsir — tanpa nama pribadi di artefak publik.
...
- **Canonical GitHub (SIDIX):** https://github.com/fahmiwol/sidix — setelah pekerjaan material (Praxis, planner, API), **commit dan push** ke `main` agar kontinuitas multi-agen terjaga; workspace lokal Windows bisa bernama `MIGHAN Model` namun isi mengikuti repo itu. Cabang fitur contoh: `sociometer-sprint7` (paket dokumen SocioMeter / sprint terkait).
