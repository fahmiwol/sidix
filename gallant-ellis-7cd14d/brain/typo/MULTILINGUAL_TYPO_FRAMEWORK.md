...
```python
# brain/typo/language_detector.py
...
```python
# brain/typo/typo_dictionaries.py
...
```python
# brain/typo/universal_corrector.py
...
```python
# brain/typo/cultural_responder.py
...
1. Pakai folder: brain/typo/ (bukan brain/typo/ — disatukan di repo)
2. Kembangkan pipeline + kamus sesuai struktur di dokumen ini
3. Isi modul kamus / locale (mis. brain/typo/locales/*.json) untuk 6 bahasa
4. Implementasikan pipeline end-to-end (MVP ada di brain/typo/pipeline.py)
5. Integrasikan ke jalur chat: apps/brain_qa/brain_qa/typo_bridge.py + run_react
...
## INTEGRASI RUNTIME (STATUS REPO)

Spesifikasi kamus besar di dokumen ini bersifat **implementation-ready**; kode produksi minimal saat ini:

| Komponen | Lokasi | Catatan |
|----------|--------|---------|
| Pipeline Unicode + substitusi ringan | `brain/typo/pipeline.py` | *Standing alone*, tanpa API vendor |
| Muat pipeline dari `brain_qa` | `apps/brain_qa/brain_qa/typo_bridge.py` | Root repo = tiga level di atas `brain_qa/typo_bridge.py` |
| Penyambungan chat | `run_react` di `apps/brain_qa/brain_qa/agent_react.py` | Setelah gate keamanan; kunci cache/RAG memakai teks ternormalisasi |
| Nonaktifkan | Env `SIDIX_TYPO_PIPELINE=0` | Teks tetap apa adanya |

Kerangka **4 lapis khusus Indonesia** (semantic matcher, confidence, context-aware) dijelaskan di **`brain/typo/TYPO_RESILIENT_FRAMEWORK.md`**. Orkestrator pilar Jiwa terpasang di **`apps/brain_qa/brain_qa/jiwa/`**; peta korpus di **`brain/jiwa/`** dan **`brain/jiwa/ARSITEKTUR_JIWA_SIDIX.md`**.
