...
def generate_3d_from_text(
    ...
    *,
    ...
    """
    Sprint 14f: text-to-3D via Shap-E (mighan-media-worker tool=3d, mode=shape).

    Berbeda dari generate_3d_from_image (TripoSR yang butuh PNG hero):
    - INPUT: text prompt langsung (tidak depend hero_mascot.png)
    - MODEL: openai/shap-e (lebih ringan dari TripoSR)
    - PATH: alternatif kalau image-to-3D gagal/cold-start lama

    Cocok sebagai fallback ketika:
    - hero_mascot.png belum/gagal di-render
    - GPU supply throttle bikin TripoSR cold-start time-out
    - Cepat preview 3D dari brief text saja

    Per /root/mighantect-3d/runpod-media-worker/media_server.py:
    - mode='shape' → ShapEPipeline.from_pretrained('openai/shap-e')
    - num_inference_steps default 64 di worker, bisa di-trim ke 32 untuk faster
    ...
    if not (prompt or "").strip():
        return {"success": False, "label": label, "error": "prompt kosong"}

    payload = {
        "input": {
            "tool": "3d",
            "prompt": prompt.strip()[:500],  # Shap-E pakai prompt, BUKAN image
            "mode": "shape",
            "output_format": output_format,
    ...
    return {
        ...
        "generation_time_s": out.get("generation_time"),
        "elapsed_ms": elapsed_ms,
        "source_prompt": prompt.strip()[:200],
    }
