...
def _google_ai_search_sync(query: str, max_results: int = 5) -> list[GoogleAIHit]:
    ...
    try:
        ...
        # Use google_search_tool if available (requires specific model + API enablement)
        try:
            google_search_tool = Tool(google_search=genai.types.GoogleSearch())
        except Exception as exc:
            log.debug("[google_ai_search] google_search_tool not available: %s", exc)
        ...
        prompt = (
            f"Cari informasi aktual dan terkini tentang: {query}\n\n"
            f"Berikan ringkasan dalam format:\n"
            f"Judul | URL | Snippet\n"
            f"Maksimal {max_results} hasil."
        )
